Feature: Question2
  ...
  Scenario: test
    ...
    Then I click the button  with xpath "//button[@type='submit']" on the page.
    Then I can check the page url contain "opensource-demo.orangehrmlive.com/web/index.php/dashboard/index".
    Then I wait for "2" seconds.
    #点击左侧菜单栏Claim
    Then I click the button  with xpath "//*[text()='Claim']/.." on the page.
    Then I wait for "2" seconds.
    #点击上方菜单栏Employee Claims
    Then I click the button  with xpath "//a[text()="Employee Claims"]" on the page.
    Then I wait for "2" seconds.
    #点击新增一条数据
    ...
    Then I wait for "2" seconds.
    #点击保存
    Then I click the button  with xpath "//button[text()=" Create "]" on the page.
    ...
    Then I wait for "2" seconds.
    #在详情页面添加扩展信息
    Then I click the button  with xpath "//h6[text()="Expenses"]/../button" on the page.
    Then I wait for "2" seconds.
    Then I select "Transport" on the element with xpath "//*[text()="-- Select --"]".
    Then I wait for "2" seconds.
    Then I click the button  with xpath "//*[@class="oxd-date-input"]/input" on the page.
    Then I wait for "2" seconds.
    Then I click the button  with xpath "//*[text()="Today"]" on the page.
    Then I wait for "2" seconds.
    When I input "888.00" into the field with xpath "//label[text()="Amount"]/../..//input" on the page.
    Then I wait for "2" seconds.
    #保存扩展信息并校验成功提示
    Then I click the button  with xpath "//button[text()=" Save "]" on the page.
    Then I wait for "2" seconds.
    Then I check the success message is "Success" with the xpath "//p[text()='Success']".
    Then I check the success message is "Successfully Saved" with the xpath "//p[text()='Successfully Saved']".
